...
class plgContentMultithumb extends JPlugin
{
    ...
    function init_params($params, $context)
    {
        ...
        $this->_params->def('disable_image_cache_link', 0);
    ...
    function set_blog_params($context)
    {
        ...
        if ($this->is_blog) {
            ...
            (($this->_params->get('enable_blogs') == 0) ||
                ($this->_params->get('enable_blogs') == 2 && /* $blog_ids && */
                    !in_array($item_id, $blog_ids)))
            ) {
...
document.write(\'<link rel="stylesheet" href="' . $this->_live_site . '/plugins/content/multithumb/greybox/gb_styles.css" type="text/css" media="screen"  />\'); ');
    ...
    function _onPrepareContent($context, &$row, &$params, $page = 0)
    {
        ...
        if ( /*preg_match ( "/^com_content./", $context ) && */
            $this->is_article && $this->_params->get('enable_thumbs') < 4
        ) {
    ...
    function image_replacer(&$matches)
    {
        ...
        if ((($this->is_article and /* preg_match('/{multithumb([^}]*)}/is', $row->text)==0 and */
                    $this->_params->get('enable_thumbs') == 4) and
...
//					}
            ...
            switch ($this->popup_type) {
                case 'normal': // Normal popup
                    /* $imgtemp  */
                    $img = "<a target=\"_blank\" href=\"$imgurl\" onclick=\"this.target='_self';this.href='javascript:void(0)';thumbWindow( '" . JURI::base(false) . "' , '$imgurl','$alt',$real_width,$real_height,0," . $this->_params->get('allow_img_toolbar') . ");\" " . $cursor_style . " >";
                    break;
                case 'iLoad':
...
';
    ...
    public static function botmt_thumbnail($filename, &$width, &$height, $proportion = 'bestfit', $bgcolor = 0xFFFFFF,
                                           $watermark = 0, $dest_folder = 'thumbs', $size_only = 0, /* $size = 0, */
                                           $img_type = "", $zoomin = 0,
                                           &$iptc_caption = NULL, plgContentMultithumb &$me = NULL)
    {
        ...
        $prefix = '';
        $prefix = substr($proportion, 0, 1) . "_" . $width . "_" . $height . "_" . $bgcolor . "_" . $watermark . "_" .(int)$zoomin . "_";
        ...
        if (isset($me)) {
            switch ($me->_params->get('scramble')) {
                case 'md5':
                    $thumb_file = md5($thumb_file) . '.' . $thumb_ext;
                    break;
                case 'crc32':
                    $thumb_file = sprintf("%u", crc32($thumb_file)) . '.' . $thumb_ext;
            }
        }
        ...
        if (file_exists($thumbname)) {
            ...
            if ( /* !$this->_params->get('disable_image_cache_link') && */ is_link(JPATH_BASE . "/images/multithumb_" . $dest_folder)) {
